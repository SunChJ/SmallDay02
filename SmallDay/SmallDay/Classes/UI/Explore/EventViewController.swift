...
class EventViewController: UIViewController {
    ...
    private func setCustomNavigationItem() {
        ...
        //添加返回按钮
        setButton(backBtn, CGRectMake(-7, 20, 44, 44), "back_0", "back_2", #selector(EventViewController.backButtonClick))
        view.addSubview(backBtn)
        // 添加收藏按钮
        setButton(likeBtn, CGRectMake(AppWidth - 105, 20, 44, 44), "collect_0", "collect_0", #selector(EventViewController.lickBtnClick))
        likeBtn.setImage(UIImage(named: "collect_2"), forState: .Selected)
        view.addSubview(likeBtn)
        // 添加分享按钮
        setButton(sharedBtn, CGRectMake(AppWidth - 54, 20, 44, 44), "share_0", "share_2", #selector(EventViewController.sharedBtnClick))
