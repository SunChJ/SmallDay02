...
class SignUpViewController: UIViewController {
    ...
    private func setUpXIB() {
        ...
        sendCodeBtn.addTarget(self, action: #selector(SignUpViewController.sendCodeBtnClick(_:)), forControlEvents: .TouchUpInside)
    }
    ...
    func sendCodeBtnClick(sender: UIButton) {
        if phoneNumTextField.text!.validateMobile() {
            ...
            self.timer = NSTimer(timeInterval: 1.0, target: self, selector: #selector(SignUpViewController.changeBtn), userInfo: nil, repeats: true)
