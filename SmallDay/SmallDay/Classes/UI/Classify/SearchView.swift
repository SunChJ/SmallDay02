...
class SearchView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        searchBtn = SearchButton(frame: CGRect(x: AppWidth - 100, y: 0, width: 100, height: 50), target: self, action: #selector(SearchView.searchBtnClick(_:)))

        addSubview(searchBtn)
        
        // 监听键盘弹出
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(SearchView.keyBoardWillshow), name: UIKeyboardWillShowNotification, object: nil)
    }
